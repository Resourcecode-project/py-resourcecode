...
def read_data_temp(fname):
    ...
    -------
    df : PANDAS DATAFRAME
        Pandas datframe contraining the data from the csv file, replicating
        dataframe created by RCODE data extraction method
    """
    custom_date_parser = lambda x: dt.datetime.strptime(x, "%d/%m/%Y %H:%M")
    df = pd.read_csv(fname,parse_dates=[0], index_col='DateTime',
                     date_parser=custom_date_parser) #parse_dates=['DateTime']
    return df

def exceed(df):
    """
    Returns ranked data and exceedence probabilities

    Parameters
    ----------
    df : PANDAS DATAFRAME
        Dataframe that contains either full data or subset (yearly or monthly)
        in a format consostent with RCODE data extraction
    ...
    exceedance = 1.-np.arange(1.,len(datasrt) + 1.)/len(datasrt)
    return datasrt,exceedance

               
def univar_monstats(df,varnm,filestats=None):
    """
    ...
    for the input variable and monthly exceedence plots
    
    Optionally, the statistics can be also written in a csv file
    ...
    filestats : STRING, optional
        Full path to the csv file where the monthly statistcs are written to. 
        The default is None.

    Returns
    -------
    dty : PANDAS DATAFRAME
        Contains yearly statistics for the chosen varaible.
    dtm : PANDAS DATAFRAME
        Contains monthly statistics for the chosen varaible..
    ...
    dtm = pd.DataFrame(np.zeros([moun.shape[0],4]),
         columns=['Min','Mean','Max','St.Dev.'],
         index=moun)
    dty = pd.DataFrame(np.zeros([yeun.shape[0],4]),
         columns=['Min','Mean','Max','St.Dev.'],
         index=yeun)
    for mo in moun:
        subs = df[varnm] \
            [df[varnm].index.month==mo]
    ...
    plt.fill_between(moun,dtm['Mean']-dtm['St.Dev.'], \
                     dtm['Mean']+dtm['St.Dev.'],alpha=0.5,label='St.Dev')
    plt.xlabel('Month')
    plt.ylabel(varnm)
    plt.legend()
    plt.show()
    ...
    plt.fill_between(yeun,dty['Mean']-dty['St.Dev.'], \
                     dty['Mean']+dty['St.Dev.'],alpha=0.5,label='St.Dev')
    plt.xlabel('Year')
    plt.ylabel(varnm)
    plt.legend()
    plt.show()
...
def bivar_stats(df,steph=0.5,stept=1):
    ...
    Produces dataframes containing percentage occurence, number occurence, 
    average ebnergy flux and energy flux standard devaition for significant
    ...
    ------
    NameError
        The inpiut dataframe should contain all the necessary data: 'hs','t0m1'
        and 'cge'
    ...
    dfprc : PANDAS DATAFRAME
        ...
        height (hs) vs energy period (t0m1). Indecies are hs binns, columns 
        are t0m1 bins
    ...
    dfcgemn : PANDAS DATAFRAME
        ...
        height (hs) vs energy period (t0m1) bins. Indecies are hs binns, columns 
        are t0m1 bins
    ...
    """
    if not all([any(df.columns=='hs'),any(df.columns=='t0m1'),any(df.columns=='cge')]):
        raise NameError('Crucial parameter missing: ''hs'',''t0m1'' or ''cge''')
    else:
        hsbin = steph*np.arange(np.floor(df['hs'].min()/steph), \
                          1+np.ceil(df['hs'].max()/steph))
        tebin = stept*np.arange(np.floor(df['t0m1'].min()/stept), \
                                1+np.ceil(df['t0m1'].max()/stept))
        ...
        for idy,hsl in enumerate(hsbin):
            for idx,tel in enumerate(tebin):
                ...
                cgesub = df['cge'].loc[(df['hs']>hsl) & (df['hs']<=(hsl+steph)) & \
                    (df['t0m1']>tel) & (df['t0m1']<=(tel+stept))].values
...
def disp_table(df,title):
    ...
    None.
    """
    pd.set_option('display.float_format','{:3,.3f}'.format, \
...
def bivar_monstats(df,filename='resource_assess',steph=0.5,stept=1,disptab=True):
    ...
    tallprc.to_csv(filename+'_All_perc_occur.csv',index=True,index_label='Hs/Te', \
                   header=True, float_format='%.1f')
    tallcnt.to_csv(filename+'_All_count.csv',index=True,index_label='Hs/Te', \
                   header=True, float_format='%.1f')
    tallcgemn.to_csv(filename+'_All_av_energy_flux.csv',index=True,index_label='Hs/Te', \
                   header=True, float_format='%.1f')
    tallcgesd.to_csv(filename+'_All_std_energy_flux.csv',index=True,index_label='Hs/Te', \
                   header=True, float_format='%.1f')
    if disptab:
    ...
    for mo in moun:
        ...
        monm = dt.datetime(2000,mo,1).strftime("%b")
        tmprc.to_csv(filename+'_'+monm+'_perc_occur.csv', \
            index=True,index_label='Hs/Te', header=True, float_format='%.1f')
        tmcnt.to_csv(filename+'_'+monm+'_count.csv', \
            index=True,index_label='Hs/Te',header=True, float_format='%.1f')
        tmcgemn.to_csv(filename+'_'+monm+'_av_energy_flux.csv', \
            index=True,index_label='Hs/Te',header=True, float_format='%.1f')
        tmcgesd.to_csv(filename+'_'+monm+'_std_energy_flux.csv', \
            index=True,index_label='Hs/Te',header=True, float_format='%.1f')
        if disptab:
